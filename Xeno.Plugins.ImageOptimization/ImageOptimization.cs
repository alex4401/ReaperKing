...
namespace Xeno.Plugins
{
        ...
        : RkModule, IRkResourceProcessorModule
    {
        ...
        public ImageOptimizationConfiguration Config { get; private set; }
            ...
            : base(typeof(RkImageOptimizationModule), site)
        {
            ...
            Site.ProjectConfig.InjectProperty<ImageOptimizationConfiguration>("modules.imageOptimization");
        }

        public override void AcceptConfiguration(ProjectConfigurationManager config)
        {
            Config = config.Get<ImageOptimizationConfiguration>();
        }
        ...
        public void ProcessResource(string filePath, ref string diskPath, ref string uri)
        {
            ...
            if (!File.Exists(optimizedPath))
            {
                ...
                switch (extension)
                {
                    case "png":
                        ...
                        if (!usesTransparency)
                        {
                            ...
                            Log.LogWarning($"Failed to convert non-transparent PNG \"{resourceKey}\" to a " +
                                           "JPEG file. Falling back to Oxipng.");
                        }
                    ...
                    default:
                        Log.LogWarning("A file extension has been included for image optimization, but no " +
                ...
                if (!success)
                {
                    Log.LogError($"Failed to optimize \"{resourceKey}\": see above for more information. " +
                                 "Original resource will be used.");
                    return;
                }
            }
